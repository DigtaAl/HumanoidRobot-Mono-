...
int main(int argc, char **argv)
{
  ...
  if (g_is_simulation == false)
  {
    ...
    int _return = packet_handler->write2ByteTxRx(port_handler, SUB_CONTROLLER_ID, 26, 992);

    if(_return != 0)
      ROS_DEBUG_STREAM("Fail to control LED ii:" << _return);
      ROS_ERROR("Fail to control LED iii[%s]", packet_handler->getRxPacketError(_return));
