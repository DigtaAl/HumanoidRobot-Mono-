...
void WalkingModule::queueThread()
{
  ...
  ros::Subscriber ypr_sub = ros_node.subscribe("sensor_ypr", 0, &WalkingModule::angleCorrection, this);

  ros::Subscriber walking_correction_sub = ros_node.subscribe("walking_correction", 0, &WalkingModule::configChangedCallback, this);
...
void WalkingModule::angleCorrection(const robotis_controller_msgs::SensorYPR::ConstPtr &ypr){
  
  
  sensorPitch = ypr->pitch*0.05 + zeroPitchOffset;
...
double WalkingModule::pidWalkXcorrection(){
  ...
  unsigned long time_diff = (unsigned long)(now - lastTimePID);
...
void WalkingModule::sendMonitorCorrection(unsigned long deltaT, double inputPitch, double correction){
...
void WalkingModule::configChangedCallback(const op3_walking_module_msgs::WalkingCorrection &walkCorrection){
  pitchPID.p = walkCorrection.wb_p_gain;
  pitchPID.i = walkCorrection.wb_i_gain;
  pitchPID.d = walkCorrection.wb_d_gain;

  xOffsetMultiplier = walkCorrection.x_offset_multiplier;
  zeroOffsetScale = walkCorrection.pitch_offset_multiplier;
  zeroPitchOffset = walkCorrection.zero_pitch_offset;

  // ROS_INFO("Result [%f]", pitchPID.p);
  // ROS_INFO("Result [%f]", pitchPID.i);
  // ROS_INFO("Result [%f]", pitchPID.d);
  
  // ROS_INFO("Result [%f]", xOffsetMultiplier);
  // ROS_INFO("Result [%f]", zeroOffsetScale);
  // ROS_INFO("Result [%f]", zeroPitchOffset);
...
{
  // double pitchKickstart = (walking_param_.x_move_amplitude - msg->x_move_amplitude) * zeroKickStartScale;
  zeroPitchOffset = walking_param_.x_move_amplitude * -zeroOffsetScale;
  // zeroPitchOffset += pitchKickstart; // 2
  walking_param_ = *msg;
}
...
void WalkingModule::updateMovementParam()
{
  ...
  // Forward/Back
  // x_move_amplitude_ = walking_param_.x_move_amplitude + (pitchPID.output * 0.03);
  ...
  // x_offset_ = walking_param_.init_x_offset + (pitchPID.output * 0.5);
  pitchPID.apply();
...
{
  //+ (pitchPID.applied * xOffsetMultiplier);
  ...
  p_offset_ = walking_param_.init_pitch_offset + (pitchPID.applied * xOffsetMultiplier);
