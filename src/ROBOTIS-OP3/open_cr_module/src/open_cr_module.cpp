...
void OpenCRModule::queueThread()
{
  ...
  ypr_pub_ = ros_node.advertise<robotis_controller_msgs::SensorYPR>("sensor_ypr", 1);
  button_pub_ = ros_node.advertise<std_msgs::String>("/robotis/open_cr/button2", 1);
                           ...
                           std::map<std::string, robotis_framework::Sensor *> sensors)
{
  ...
  // ROS_INFO("Filtered : %f, %f, %f", result_["acc_x"], result_["acc_y"], result_["acc_z"]);
...
void OpenCRModule::publishIMU()
{
  ...
  ypr_msg_.roll = result_["roll"];
  ypr_msg_.pitch = result_["pitch"];
  ypr_msg_.yaw = result_["yaw"];
...
void OpenCRModule::publishButtonMsg(const std::string &button_name)
{
  ...
  publishStatusMsg(robotis_controller_msgs::StatusMsg::STATUS_INFO, "Button : pressed");
